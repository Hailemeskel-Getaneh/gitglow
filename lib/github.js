...
async function fetchStats(username, token, countPrivate = false) {
  const cacheKey = `stats-${username}-${countPrivate}`;
  ...
  try {
    // Run Search and GraphQL in parallel for maximum speed
    const [gqlRes, commitRes, prRes, issueRes] = await Promise.all([
      axios.post('https://api.github.com/graphql', { query, variables: { login: username } }, { headers: { Authorization: `bearer ${token}` } }),
      axios.get(`https://api.github.com/search/commits?q=author:${username}`, { headers: { Authorization: `token ${token}`, Accept: 'application/vnd.github.cloak-preview+json' } }).catch(() => ({ data: { total_count: 0 } })),
      axios.get(`https://api.github.com/search/issues?q=author:${username}+type:pr`, { headers: { Authorization: `token ${token}` } }).catch(() => ({ data: { total_count: 0 } })),
      axios.get(`https://api.github.com/search/issues?q=author:${username}+type:issue`, { headers: { Authorization: `token ${token}` } }).catch(() => ({ data: { total_count: 0 } }))
    ]);
    ...
    const stats = {
      ...
      username: user.login,
      stars: totalStars,
      commits: commitRes.data.total_count || 0,
      prs: prRes.data.total_count || 0,
      issues: issueRes.data.total_count || 0,
 ...
 */
async function fetchStreak(username, token, countPrivate = false) {
    const cacheKey = `streak-${username}-${countPrivate}`;
    const cachedData = cache.get(cacheKey);
    if (cachedData) return cachedData;
    ...
    try {
        const [gqlRes, commitRes] = await Promise.all([
            axios.post('https://api.github.com/graphql', { query, variables: { login: username } }, { headers: { Authorization: `bearer ${token}` } }),
            axios.get(`https://api.github.com/search/commits?q=author:${username}`, { headers: { Authorization: `token ${token}`, Accept: 'application/vnd.github.cloak-preview+json' } }).catch(() => ({ data: { total_count: 0 } }))
        ]);
        ...
        // Use Search API total if it's higher than the annual calendar total
        const lifetimeCommits = commitRes.data.total_count || 0;
        const displayTotal = Math.max(calendar.totalContributions, lifetimeCommits);
        ...
        cache.set(cacheKey, streakResult);
        return streakResult;
    } catch (error) {
